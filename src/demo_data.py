...
def generate_synthetic_posts(
    ...
    if eligible.empty:
        raise ValueError("No influencers with >= 1000 followers available for synthetic data.")

    if target_posts is not None:
        needed_influencers = min(len(eligible), max(num_influencers, target_posts // max_posts))
        eligible = eligible.sample(n=needed_influencers, random_state=seed)
    elif len(eligible) > num_influencers:
        eligible = eligible.sample(n=num_influencers, random_state=seed)

    base_timestamp = 1_700_000_000
    rows: list[dict] = []
    influencer_states: dict[int, dict] = {}

    for influencer_idx, influencer in eligible.iterrows():
        influencer_states[influencer_idx] = {
    ...
    if target_posts is not None:
        influencer_order = list(influencer_states.keys())
        cursor = 0
        while len(rows) < target_posts:
            influencer_idx = influencer_order[cursor % len(influencer_order)]
            state = influencer_states[influencer_idx]
            post_idx = state["post_count"]
            rows.append(
                _make_synthetic_post_row(
                    state["influencer"],
                    influencer_idx,
                    post_idx,
                    rng,
                    ...
                    state["preferred_media"],
                )
            )
            state["post_count"] += 1
            cursor += 1
    else:
        for influencer_idx, state in influencer_states.items():
...
def build_posts_base_from_parquet_or_synthetic(
    ...
    if posts_parquet:
        ...
        if "log_engagement_score" in posts_df.columns and "followers" in posts_df.columns:
            return posts_df
    ...
    if synthetic:
        return build_synthetic_posts_base(
            influencers_path,
            num_influencers=num_influencers,
            target_posts=target_posts,
            seed=seed,
